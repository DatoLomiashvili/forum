...
it('redirects to the post show page', function () {
    ...
    actingAs($comment->user)
        ->put(route('comments.update', $comment), ['body' => fake()->word])
...
it('redirects to the correct page of comments', function () {
    ...
    actingAs($comment->user)
        ->put(route('comments.update', ['comment' => $comment, 'page' => 2]), ['body' => fake()->word])
...
it('cannot update a comment from another user', function () {
    ...
    actingAs(User::factory()->create())
        ->put(route('comments.update', ['comment' => $comment]), ['body' => fake()->word])
        ->assertForbidden();
});
